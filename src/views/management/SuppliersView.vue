...
<template>
  ...
  <v-card v-if="mdAndUp" elevation="0" class="mb-10 pa-4">
    <v-row>
      ...
      <v-col cols="12" md="2" class="d-flex justify-end align-center" hide-details>
        <v-btn prepend-icon="mdi-plus" color="primary" @click="supplierFormModal = true" v-role="[ROLES.ADMIN]">Crear
          Proveedor</v-btn>
      </v-col>
    </v-row>
  </v-card>
  ...
  <v-data-table :headers="headers" :items="filtroProveedores" :search :loading="isPending"
    ...
    <template #[`item.actions`]="{ item }">
      ...
      <ActionMenu :onEdit="auth.hasRole(ROLES.ADMIN) ? () => handleEdit(item) : null"
